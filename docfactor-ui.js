...
function setTheme(theme) {
  ...
  localStorage.setItem("hyperspeed_theme", theme);
  const button = $("themeToggle");
  if (button) button.textContent = theme === "dark" ? "Light" : "Dark";
}

function initTheme() {
  setTheme(localStorage.getItem("hyperspeed_theme") || "light");
...
async function loadCustomers() {
  ...
  for (const select of selects) {
    ...
    select.innerHTML = state.customers.length
      ? state.customers.map((c) => `<option value="${c.id}">${htmlEscape(c.id)} - ${htmlEscape(c.name)}</option>`).join("")
