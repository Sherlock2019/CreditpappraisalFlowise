...
start_flowise() {
  ...
  if command -v flowise >/dev/null 2>&1; then
    log "Starting Flowise on http://localhost:${FLOWISE_PORT}"
    PORT="$FLOWISE_PORT" flowise start >"${POC_DIR}/logs/flowise.log" 2>&1 &
    FLOWISE_PID=$!
  elif command -v npx >/dev/null 2>&1; then
    log "Starting Flowise with npx on http://localhost:${FLOWISE_PORT}"
    PORT="$FLOWISE_PORT" npx --yes flowise start >"${POC_DIR}/logs/flowise.log" 2>&1 &
    FLOWISE_PID=$!
  else
...
start_launcher() {
  ...
  "$PYTHON_BIN" -m http.server "$LAUNCHER_PORT" --bind 127.0.0.1 >"${APP_DIR}/web.log" 2>&1 &
  LAUNCHER_PID=$!
}
